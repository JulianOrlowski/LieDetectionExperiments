...
<html xmlns="http://www.w3.org/1999/xhtml">
    ...
    <body onload="Init();">
        ...
        if (isset($_POST["ques_submit"])){
            ...
            if(trim($text_Answer)==null){
                echo "<script>alert('La risposta non può essere nulla');location='experiment_review.php';</script>";
                mysqli_close($conn);
            }
            else if (trim(is_numeric($text_Answer))&&$letters_number==0) {
                echo "<script>alert('the answer can not be writen in numbers');location='experiment_review.php';</script>";
                mysqli_close($conn);
            }
            else{              
		...
		$classifier_file = "../data_analysis/classifier_implementation.py";
		if (!(file_exists("features.csv"))){
		   $cmd = escapeshellcmd("python3 ../data_analysis/real_features_extraction.py all");
		   shell_exec($cmd);
		 }
		 $cmd = escapeshellcmd("python3 ../data_analysis/real_features_extraction.py ". $session_ID_Post);
		 $output = shell_exec($cmd);

		 $train_file="features.csv";
		 $test_file="features_one_session.csv";

		 $cmd = escapeshellcmd("python3 " . $classifier_file . " " . $classifier . " " .$train_file . " " . $test_file);
		 $output = shell_exec($cmd);
		 echo $output;
        ...
        //load the content of the table
            ...
            $question_ID = $mindCondition;
            $sql_find_Question= mysqli_query($conn, "select * from questions_long where question_id= $question_ID");
            $rs_find_Question = mysqli_fetch_array($sql_find_Question, MYSQLI_BOTH);
            if($rs_find_Question){
                ?>
                ...
                <form action="" method="post" name="ques_form" id="ques_form"  style="margin-bottom:0px;" autocomplete="off" class="form-horizontal">
                    <div class="text-center" >
                            <h3><?php echo htmlspecialchars($rs_find_Question['text_short']); ?>
                            </h3>
                    <?php                              
            }
                    ?>
                    ...
                    <div class="col-xs-offset-0 col-xs-12 form-group top-margins bottom-margins">
                        ...
                        <label for="productRate" class="col-xs-7 control-label">Valutazione complessiva:<font color="#FF0000">*</font></label>
                            <div class="col-xs-10 ">
                                <label class="radio-inline">
                                <input name="productRate" type="radio" id="1" value="1" checked="checked" />1</label>
                                <label class="radio-inline">
                                <input type="radio" name="productRate" value="2" id="2" />2</label>
                                <label class="radio-inline">
                                <input type="radio" name="productRate" value="3" id="3" />3</label>
                                <label class="radio-inline">
                                <input type="radio" name="productRate" value="4" id="4" />4</label>
                                <label class="radio-inline">
                                <input type="radio" name="productRate" value="5" id="5" />5</label>
                            </div>
                        </div>
                        <div class="form-group">
                            <label for="reviewTitle" class="col-xs-7 control-label">Aggiungi un titolo:<font color="#FF0000">*</font></label>
                            <div class="col-xs-9 ">
                                <input class="form-control" name="reviewTitle" type="text" id="reviewTitle" autofocus="autofocus"/>
                            </div>
                        </div> 
                        <div class="form-group">
                            <label for="review" class="col-xs-7 control-label">Scrivi la tua recensione:<font color="#FF0000">*</font></label>
                            <div class="col-xs-12 ">
                            <textarea placeholder="Inserisci qui i commenti sulla tua esperienza con questo venditore" rows="8" cols="150" class="form-control col-xs-10" id="answerContent" name="answerContent" onclick="set_timestamp_tap()"></textarea>
                            </div>
                        </div>
                   ...
                   <div class="col-xs-offset-1 col-xs-10">
                    <input type="submit" id="ques_submit" name="ques_submit" value="Next" onclick="answer_submission()"/>
                </form>
            </div>
    ...
    <script>
        ...
        //boolean to detect typing errors
        var typing_errors = 0;
        ...
        function Init() {
            var textarea = document.getElementById ("answerContent");
            if (textarea != null){
        ...
        function OnInput (event) {
            if (last_string == null){
                last_string = event.target.value;
            }
            else if (new_string == null){
                new_string = event.target.value;
            }
            else{
                last_string = new_string;
                new_string = event.target.value;
            }
            computeKeyTyped(last_string, new_string);
        ...
        function computeKeyTyped(last_string, new_string){
            if (new_string == null){
                typed_keycode = last_string[last_string.length - 1].charCodeAt(0);
            }
            else if (new_string.length < last_string.length){
                typed_keycode = 8;
            }
            else{
                typed_keycode = new_string[new_string.length - 1].charCodeAt(0);
            }
        }
        ...
        //initializing the keyUP listener	
        $('#answerContent').keyup(function(evt) {
            ...
            if(typed_keycode == 13) { // enter answer submission
                //showDynamic();
                set_timestamp(3);
                //answer_submission();
            ...
            if( typed_keycode == 8 ) {//backspace
                typing_errors++;
            }
            if (evt.which == 229){
                character_typed=String.fromCharCode(typed_keycode);
                keydyn.push({"t":evt.timeStamp,"tn":time_now,
                    "cod":typed_keycode, "character":character_typed,
                    "k":'UP'});
                console_log("KEYBOARD: Key code: "+typed_keycode+" character "+character_typed+" UP "+" on time: "+evt.timeStamp);
            }
            else{
                character_typed=String.fromCharCode(evt.which);
                keydyn.push({"t":evt.timeStamp,"tn":time_now,
